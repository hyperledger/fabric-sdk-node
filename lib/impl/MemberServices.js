...
var MemberServices = class extends api.MemberServices {
	...
	constructor(url, opts) {
		...
		logger.info('Successfully constructed member service client: endpoint - %j, crypto - %s/%s/%s',
			this._endpoint,
			this.cryptoPrimitives.getPublicKeyAlgorithm(),
			this.cryptoPrimitives.getSecurityLevel(),
			this.cryptoPrimitives.getHashAlgorithm());
	}
	...
	register(req, registrar) {
		...
		return new Promise(function(resolve, reject) {
			...
			var signKey = self.cryptoPrimitives.getKeyPairForSigning(registrar.getEnrollment().privateKey, 'hex');
		...
		return new Promise(function(resolve, reject) {
			...
			self._ecapClient.createCertificatePair(eCertCreateRequest, function (err, eCertCreateResp) {
				...
				self._ecapClient.createCertificatePair(eCertCreateRequest, function (err, eCertCreateResp) {
					...
					var enrollment = new api.Enrollment(signingKeyPair.prvKeyObj.prvKeyHex, eCertCreateResp.certs.sign.toString('hex'), eCertCreateResp.pkchain.toString('hex'));
