...
//		logger.debug('Member.sendTransaction - chaincodePropsoal %j', chaincodeProposal);
	...
	sendDeploymentProposal(request) {
		...
		return packageChaincode(request.chaincodePath, request.chaincodeId)
		.then(
			function(data) {
				...
				return new Promise(function(resolve, reject) {
					fs.readFile(targzFilePath, function(err, data) {
						if(err) {
							...
							var proposal =  Member._buildProposal(lcccSpec);
	...
	sendTransactionProposal(request) {
		...
		var proposal =  Member._buildProposal(invokeSpec);
	 ...
	 */
	static _buildProposal(invokeSpec) {
...
function packageChaincode(chaincodePath, chaincodeId, args) {
