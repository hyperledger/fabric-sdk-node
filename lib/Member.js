...
//		logger.debug('Member.sendTransaction - chaincodePropsoal %j', chaincodeProposal);
		...
		let header = Member._buildHeader('somebody', null);
				...
				return new Promise(function(resolve, reject) {
					fs.readFile(targzFilePath, function(err, data) {
						if(err) {
							...
							let proposal = Member._buildProposal(lcccSpec, 'lccc');

							return Member._sendPeersProposal(request.targets, proposal)
							.then(
								function(responses) {
	...
	sendTransactionProposal(request) {
		...
		let proposal = Member._buildProposal(invokeSpec, request.chaincodeId);
...
//			headerExt.setPayloadVisibility(TODO);
	 ...
	 * @private
	 */
	static _buildProposal(invokeSpec, chaincode_id) {
...
//		cciSpec.setIdGenerationAlg('');
		...
		let header = this._buildHeader('somebody', chaincode_id);
...
//		signedProposal.setProposalBytes(proposal.toBuffer());
//		return signedProposal;
	...
	fromString(str) {
		...
		var self = this;
		var crypto = sdkUtils.getCryptoSuite();
		...
		var promise = crypto.getKey(this._enrollment.privateKey)
		.then(function(key) {
