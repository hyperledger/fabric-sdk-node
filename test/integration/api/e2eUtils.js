...
function invokeChaincode(userOrg, version,   useStore,_data){
	init();
	...
	return promise.then((store) => {
		...
		logger.info('*****************************************************************************');
        logger.info('stop and start the peer event hub ---- N  O  W ----- you have ' + sleep_time + ' millis');
        logger.info('*****************************************************************************');
		...
		if (all_good) {
			...
			if(all_good){
                ...
                logger.info(' All proposals do not have matching read/write sets');
			}
		}
		if (all_good) {
				...
				let txPromise = new Promise((resolve, reject) => {
					...
					eh.registerTxEvent(deployId.toString(),
						(tx, code) => {
							...
							if (code !== 'VALID') {
                                logger.info('The balance transfer transaction was invalid, code = ' + code);
								reject();
							} else {
			...
			.then((results) => {
				...
				return results[0]; // the first returned value is from the 'sendPromise' which is from the 'sendTransaction()' call
                    ...
                    logger.info('Failed to send transaction and get notifications within the timeout period.');
				throw new Error('Failed to send transaction and get notifications within the timeout period.');

			});

		} else {
            logger.info('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
			throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		}
	}, (err) => {

        logger.info('Failed to send proposal due to error: ' + err.stack ? err.stack : err);
		...
		if (response.status === 'SUCCESS') {
            ...
            logger.info('******************************************************************');
            logger.info('To manually run /test/integration/query.js, set the following environment variables:');
            logger.info('export E2E_TX_ID='+'\''+tx_id.getTransactionID()+'\'');
            logger.info('******************************************************************');
			logger.debug('invokeChaincode end');
			return true;
		} else {
            logger.info('Failed to order the transaction. Error code: ' + response.status);
			throw new Error('Failed to order the transaction. Error code: ' + response.status);
		}
	}, (err) => {

        logger.info('Failed to send transaction due to error: ' + err.stack ? err.stack : err);
...
function queryChaincode(org, version, value,   transientMap) {
	init();
