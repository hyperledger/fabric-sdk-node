...
export async function createGateway(ccp: CommonConnectionProfileHelper, tls: boolean, userName: string, orgName: string, gatewayName: string, useDiscovery: boolean, walletType: string): Promise<void> {
	...
	if (!wallet) {
		...
		if (walletType === Constants.HSM_WALLET) {
			const hsmProvider2 = wallet.getProviderRegistry().getProvider(HSM_PROVIDER);
			const cryptoSuite: any = hsmProvider2.getCryptoSuite();
			cryptoSuite.closeSession();
			cryptoSuite.finalize();

			// tslint:disable-next-line
			console.log('reusing HSM Wallet');
...
async function createHSMUser(wallet: Wallet, ccp: CommonConnectionProfileHelper, orgName: string, userName: string): Promise<void> {
	...
	const identity: HsmX509Identity = {
		credentials: {
			...
			identifier: enrollment.key.getSKI()
		},
		mspId: orgMsp,
		type: HSM_PROVIDER
	};
