...
Feature: Configure Fabric using CLI and submit/evaluate using a network Gateway with discovery enabled
	...
	Scenario: Using a Gateway I recieve useful error messages when I submit or evaulate invalid transactions
		...
		Then The gateway named myDiscoveryGateway has a error type response containing "Error: Endorsement has failed"
		...
		Then The gateway named myDiscoveryGateway has a error type response containing "Error: You've asked to invoke a function that does not exist: noSuchEvauateTransaction"
		...
		Then The gateway named myDiscoveryGateway has a error type response containing "Error: Expected 1 parameters, but 4 have been supplied"
	...
	Scenario: Using a Gateway to submit transactions I can use different event handler strategies
		...
		Then The gateway named myDiscoveryGateway has a event type response matching "{\"Org1\":1,\"Org2\":0}"
