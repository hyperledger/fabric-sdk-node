...
Feature: Configure Fabric using CLI and submit/evaluate using a network Gateway without discovery
	...
	Background:
 	...
 	Scenario: Using a Gateway I can submit and evaluate transactions on instantiated node smart contract
	    ...
	    When I use the gateway named mygateway to submit a transaction with args [createCar,1001,Trabant,601 Estate,brown,Simon] for contract fabcar instantiated on channel gatewaychannel
	    Then The gateway named mygateway has a submit type response
	 	When I use the gateway named mygateway to evaluate a transaction with args [queryCar,1001] for contract fabcar instantiated on channel gatewaychannel
