...
async function create_channels(channelTxPath, ccp, tls) {
	...
	try {
		for (const channelName in channels) {
			await create_channel(channelName, channelTxPath, ccp, tls);
		}
	} catch (err) {
...
async function create_channel(channelName, channelTxPath, ccp, tls, fileName) {
	...
	let channelFileName = channelName + '.tx';
	if (fileName) {
		channelFileName = fileName;
	}
	const envelope_bytes = fs.readFileSync((path.join(channelTxPath, channelFileName)));
