...
const FabricCAServices = rewire('fabric-ca-client/lib/FabricCAClientImpl');
 ...
 */
//test constructor
test('FabricCAClient: Test constructor', (t) => {
...
const ecertPEM = fs.readFileSync(path.resolve(__dirname, '../fixtures/fabricca/ecert.pem'));

test('FabricCAClient: Test _pemToDer static method', (t) => {
...
test('FabricCAServices:  Test optional constructor', (t) => {
	const opts = {
...
test('FabricCAServices:  Test getCryptoSuite() function', (t) => {
	const ca = new FabricCAServices('http://localhost:7054');
	const crypto = ca.getCryptoSuite();
...
test('FabricCAServices: Test register() function', (t) => {
	const cop = new FabricCAServices('http://localhost:7054');
	...
	t.throws(
		() => {
			const registrar = new User('testUser3');
			cop.register({ enrollmentID: 'testUser3' }, registrar);
		},
 ...
 */
test('FabricCAServices: Test enroll with missing parameters', (t) => {
...
test('FabricCAServices: Test revoke() function', (t) => {
	const cop = new FabricCAServices('http://localhost:7054');
	t.throws(
		() => {
			cop.revoke();
	...
	t.doesNotThrow(
		() => {
			...
			return cop.revoke({ enrollmentID: null, aki: 'someId', serial: 'someId', reason: 0 }, registrar)
				.then(() => {
...
const VALID_CERT1 = '-----BEGIN CERTIFICATE-----\n' +
	'MIICEDCCAbagAwIBAgIUXoY6X7jIpHAAgL267xHEpVr6NSgwCgYIKoZIzj0EAwIw' +
	'fzELMAkGA1UEBhMCVVMxEzARBgNVBAgTCkNhbGlmb3JuaWExFjAUBgNVBAcTDVNh' +
	'biBGcmFuY2lzY28xHzAdBgNVBAoTFkludGVybmV0IFdpZGdldHMsIEluYy4xDDAK' +
	'BgNVBAsTA1dXVzEUMBIGA1UEAxMLZXhhbXBsZS5jb20wHhcNMTcwMTAzMDEyNDAw' +
...
const VALID_CERT2 = '-----BEGIN CERTIFICATE-----\n' +
...
test('FabricCAServices: Test reenroll() function', (t) => {
	const cop = new FabricCAServices('http://localhost_bad:7054');
	...
	const getSubjectCommonName = FabricCAServices.__get__('getSubjectCommonName');
...
test('FabricCAServices: Test static method normalizeX509()', (t) => {
...
function testNormalizer(cert, t) {
	const normalized = FabricCAServices.normalizeX509(cert);
...
test('FabricCAServices: Test _parseURL() function', (t) => {

	const goodHost = 'www.example.com';
	const goodPort = 7054;
	const goodURL = 'http://' + goodHost + ':' + goodPort;
	const goodURLSecure = 'https://' + goodHost + ':' + goodPort;
	const goodUrlNoPort = 'https://' + goodHost;

	const badHost = '';
	const badURL = 'http://' + badHost + ':' + goodPort;
	const badURL2 = 'httpD://' + goodHost + ':' + goodPort;
	const badURL3 = 'httpsD://' + goodHost + ':' + goodPort;
	//const badURL4 = goodHost + ':' + goodPort;
	const badURL5 = 'ftp://' + goodHost + ':' + goodPort;


	t.plan(12);

	//valid http endpoint
	const endpointGood = FabricCAServices._parseURL(goodURL);
	...
	const endpointGoodSecure = FabricCAServices._parseURL(goodURLSecure);
	...
	const endpointGoodUrlNoPort = FabricCAServices._parseURL(goodUrlNoPort);
...
test('FabricCAServices: Test toString() function', (t) => {
	const ca = new FabricCAServices('http://localhost:7054');
	const printableCa = ca.toString();
 ...
 */
test('FabricCAClient: Test enroll with missing parameters', (t) => {
 ...
 */
test('FabricCAClient: Test register with missing parameters', (t) => {
 ...
 */
test('FabricCAClient: Test revoke with missing parameters', (t) => {
